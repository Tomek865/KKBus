...
export default function AdminDashboard() {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchDashboardData = async () => {
      setLoading(true);
      try {
        // Funkcja pomocnicza: łapie błędy pojedynczego fetcha, żeby nie wysadzić całego Promise.all
        const safeFetch = async (url: string) => {
            try {
                const res = await authFetch(url);
                if (!res || !res.ok) return null; // Jeśli endpointu nie ma (np. 404), zwraca null
    ...
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flexDirection: 'row', gap: 20, marginBottom: 30 }}>
        ...
        <View style={{ flex: 2, gap: 20 }}>
            {/* Widok Floty - ograniczenie wysokości do ok. 7 elementów (420px) */}
            <View style={[styles.card, { flex: 1 }]}>
        ...
        {/* PRAWA KOLUMNA: SKRÓTY NAWIGACYJNE, PROGRAM LOJALNOŚCIOWY I NAGRODY */}
        <View style={{ flex: 1, gap: 20 }}>
            
            {/* PRZEKIEROWANIE DO ROZKŁADÓW */}
            ...
            {/* PANEL: TOP 3 UŻYTKOWNIKÓW (PROGRAM LOJALNOŚCIOWY) */}
            <View style={[styles.card, { flex: 1 }]}>
      ...
      {/* DOLNY RZĄD: TRASY I BAZA PRZYSTANKÓW */}
