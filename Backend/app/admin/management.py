...
def delete_user(current_admin_id, user_id):
    """
    Endpoint usuwa użytkownika na podstawie jego ID z frontendu (np. 'C_1' dla klienta, 'E_2' dla pracownika).
    """
    ...
    try:
        ...
        if user_type == 'C':
            cur.execute("DELETE FROM Client WHERE client_id = %s", (actual_id,))
        elif user_type == 'E':
            if int(actual_id) == current_admin_id:
                return jsonify({"error": "You cannot delete your own account"}), 403
            cur.execute("DELETE FROM Employee WHERE employee_id = %s", (actual_id,))
        else:
